    ...
    : diagnostic_(node)
{
  diagnostic_updater::TaskFunction func = std::bind(
      &TwistMuxDiagnostics::diagnostics, this, std::placeholders::_1);
  diagnostic_.add("Twist mux status", func);
  diagnostic_.setHardwareID("none");

  // not passing status_type pointer causes undefined process termination
  // when looping in TwistMuxDiagnostics::diagnostics function
...
void TwistMuxDiagnostics::diagnostics(diagnostic_updater::DiagnosticStatusWrapper &stat)
{
  ...
  for (const auto &velocity_h : *status_.velocity_hs) {
    stat.addf("velocity " + velocity_h.getName(),
              ...
              velocity_h.getTopic().c_str(), velocity_h.getTimeout(),
  ...
  for (const auto &lock_h : *status_.lock_hs) {
    stat.addf("lock " + lock_h.getName(),
              ...
              lock_h.getTopic().c_str(), lock_h.getTimeout(),
